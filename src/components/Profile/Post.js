...
function Post(props) {
    ...
    /*    useEffect(()=>{
            ...
            const height = postImageRef.current.clientHeight;
            postBodyRef.current.style.height = `${height}px`;
        },[]);*/

    return (
        <>
            <Grid item md={6} className={classes.imageContainer}>
                <img src={props.post.img} className={classes.image} ref={postImageRef}/>
            </Grid>
